...
x = mean(x,2); x = x(5001:5001+44100*5);
[tfr, ceps, tceps, tfrr, rtfr, tfrsq, tfrtic] = CFPH(x - mean(x), basicTF, advTF, cepR, P);
fs = basicTF.fs;
...
subplot(222)
imageSQ(0:time_stamp:time_stamp*(size(rtfr,2)-1), tfrtic*basicTF.fs, tfrsq, 0.999); axis xy;
%title('SST');
xlabel('time (s)'); ylabel('freqnency (s)');

subplot(223)
imageSQ(0:time_stamp:time_stamp*(size(rtfr,2)-1), tfrtic*basicTF.fs, tceps, 0.999); axis xy; colormap(1-gray);
