...
x1 = 1 * cos(2*pi*t) ; x1(60001:end) = 0 ;
x2 = AM2 .* cos(2*pi*phi) ;
...
subplot(234)
imageSQ(0:time_stamp:time_stamp*(size(rtfr,2)-1), tfrtic*basicTF.fs, abs(ceps), 0.998); axis xy; colormap(1-gray);
xlabel('time (s)'); ylabel('quefency (p)');

subplot(235)
imageSQ(0:time_stamp:time_stamp*(size(rtfr,2)-1), tfrtic*basicTF.fs, abs(tceps), 0.998); axis xy; colormap(1-gray);
xlabel('time (s)'); ylabel('frequency (Hz)');

subplot(236)
imageSQ(0:time_stamp:time_stamp*(size(rtfr,2)-1), tfrtic*basicTF.fs, tfrr, 0.998); axis xy; colormap(1-gray);
xlabel('time (s)'); ylabel('frequency (Hz)');
