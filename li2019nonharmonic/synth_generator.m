...
up.mods = {'original','bw', 'am', 'fm','bwamfm'};
...
function [ppg_data] = load_sample_beat_data
...
function [ppg_data] = generate_synthetic_data(ppg_data, up)
    ...
    for const = {'hr', 'rr'}
        ...
        for mod_no = 1 : length(up.mods)
            ...
            for hr_no = 1 : length(settings.hr.w)
                ...
                [ppg_data] = find_unmod_data(ppg_data, temp.hr, up);
...
function [ppg_data] = find_unmod_data(ppg_data, hr, up)
...
switch mod
    ...
    case 'bwamfm'
        ...
        temp.v = unmod_data.v;
        mod_data.t = unmod_data.t;
        mod_data.v = interp1(temp.t, temp.v, mod_data.t).*((1/up.a_r)+cos(w_r*unmod_data.t))+ up.a_r*sin(w_r*unmod_data.t);
end
...
function export_to_common_format(ppg_data, up)

fprintf('\n - Saving data');

%% Create a data structure based on the Karlen data format

for subj_el = 1:length(ppg_data.mod.v)
