/*
 * [Anew, Loss_new, iters] = mexQuadNN(G,W,A,optTol)
 *
 * ---Input---
 *
 ...
 * for projection onto the non-negative orthant.
 *
...
/* project x to the non-negative orthant -> output f */
inline void ProjNN(double* f, double* x, int m) {
	for(int i = 0; i< m; i++){
		if (x[i] < 0)
			f[i] = 0;
		else
...
int QuadNN(double* G, double* w, double* a0, ptrdiff_t k,
...
void spawn_threads(double* G, double* W, double* A, ptrdiff_t k, int d, double* Anew, double* Loss_new, double* iters, double optTol) {
    ...
    for(j = 0; j < d; j++){
       // int id = omp_get_thread_num();
		int id = 0;
        iters[j] = QuadNN(G, W + j * k, A + j * k, k,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    /* [Anew, Loss_new] = mexQuadNN(G,W,A) - d independent subproblems; */
    if (nrhs != 4){
        mexErrMsgTxt("Must have 4 input argument: [Anew, Loss_new, iters] = mexQuadNN(G,W,A,optTol)");
    }
    if (nlhs != 3){
        mexErrMsgTxt("Must have 3 output argument: [Anew, Loss_new, iters] = mexQuadNN(G,W,A,optTol)");
    }
	...
	for( int i = 0; i < k*d; i++){
		if( A[i] < 0 ){
			mexErrMsgTxt("A contains negative entries.");
		}
	}
